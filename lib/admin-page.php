<div class="wrap">
	...
	<form method="post" action="<?php echo admin_url('admin.php?page=hb-hooks-browser'); ?>">
		...
		<div class="form-table">
			...
			<div class="hb-field-wrap">
				...
				<div class="hb-single-field">
					<select type="select" name="hb_where_to_look" id="hb_where_to_look">
						<option value="both" <?php selected( $_POST['hb_where_to_look'], 'both' ); ?> ><?php echo __('Everywhere', 'hb'); ?></options>
						<option value="plugins"  <?php selected( $_POST['hb_where_to_look'], 'plugins' ); ?>><?php echo __('Plugin', 'hb'); ?></options>
						<option value="themes" <?php selected( $_POST['hb_where_to_look'], 'themes' ); ?>><?php echo __('Theme', 'hb'); ?></options>
					</select>
				</div>
			</div>
			...
			<div class="hb-field-wrap">
				<label class="hb-label">
					<?php echo __('Actions or Filters ?', 'hb'); ?>
				</label>
				<div class="hb-single-field">
					<select type="select" name="hb_actions_or_filters" id="hb_actions_or_filters">
						<option value="all" <?php selected( $_POST['hb_actions_or_filters'], 'all' ); ?> ><?php echo __('All', 'hb'); ?></options>
						<option value="do_action" <?php selected( $_POST['hb_actions_or_filters'], 'do_action' ); ?>><?php echo __('Defined Actions', 'hb'); ?></options>
						<option value="apply_filters" <?php selected( $_POST['hb_actions_or_filters'], 'apply_filters' ); ?>><?php echo __('Defined Filters', 'hb'); ?></options>
						<option value="add_action" <?php selected( $_POST['hb_actions_or_filters'], 'add_action' ); ?>><?php echo __('Used Actions', 'hb'); ?></options>
						<option value="add_filter" <?php selected( $_POST['hb_actions_or_filters'], 'do_filter' ); ?>><?php echo __('Used Filters', 'hb'); ?></options>
					</select>
				</div>
			</div>
